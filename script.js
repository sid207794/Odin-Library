...
newBook.addEventListener("click", () => {
  ...
  formElement.addEventListener("submit", function (event) {
    if (!inputName.validity.valid) {
      ...
      pagesValidate();
    } else if (!inputRead.validity.valid) {
      readValidate();
    } else {
      ...
      spanAuthor.style.color = "white";
      spanPages.style.color = "white";
  ...
  function authorValidate() {
    ...
    } else if (inputAuthor.validity.typeMismatch) {
      spanAuthor.textContent = "Not Text";
    }
  }

  function pagesValidate() {
    if (inputPages.validity.valueMissing) {
      spanPages.style.color = "red";
    } else if (inputPages.validity.typeMismatch) {
      spanPages.textContent = "Not Num";
    } else if (inputPages.validity.tooShort) {
      spanPages.textContent = "Too Small";
    }
  }

  function readValidate() {
    if (inputRead.validity.valueMissing) {
      ...
      deletefinalR.forEach((cardR) => {
        cardR.addEventListener("click", () => {
          if (bookReceipt.children.length === 1) {
            ...
            let cryptoTextR = document.querySelector(
              `#${gParentR} .cryptoText`
            );
            let cryptoTextContentR = cryptoTextR.textContent;
            console.log(gParentR);

            const itemIndex = myLibrary.findIndex(
              (item) => item.id === cryptoTextContentR
            );
    ...
    deletefinalHome.forEach((cardH) => {
      cardH.addEventListener("click", () => {
        if (bookDetail.children.length === 1) {
          ...
          const itemIndexH = myLibrary.findIndex(
            (itemH) => itemH.id === cryptoTextContentH
          );
